...
class BulkEmbedWebhookLogger(EmbedWebhookLogger):
    ...
    async def _loop(self):
        ...
        self._webhook = Webhook.from_url(
            self._webhook_url, adapter=AsyncWebhookAdapter(self._session))

        while True:

            with suppress(HTTPException):
                while self._to_log:
                    embeds = [self._to_log.pop(0) for _ in range(
                        min(10, len(self._to_log)))]
